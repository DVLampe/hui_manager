...
export default function HuiPage() {
  ...
  return (
    <div>
      ...
      <div className="flex border border-gray-200 rounded-lg p-1 bg-gray-50 w-max mb-6">
        <button
          onClick={() => setViewMode('tham_gia')}
          className={`px-4 py-2 rounded-md text-sm font-semibold transition-all ${viewMode === 'tham_gia' ? 'bg-white text-red-600 shadow' : 'text-gray-600'}`}
        >
          Hụi tham gia
        </button>
        <button
          onClick={() => setViewMode('lam_chu')}
          className={`px-4 py-2 rounded-md text-sm font-semibold transition-all ${viewMode === 'lam_chu' ? 'bg-white text-red-600 shadow' : 'text-gray-600'}`}
        >
          Hụi làm chủ
        </button>
      </div>
